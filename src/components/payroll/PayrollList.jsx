          ...
          filteredPayrolls.map((p) => (
            <div key={p.id} className="bg-white rounded-lg shadow p-4">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-base font-semibold text-gray-900">{p.employee_name}</p>
                  <p className="text-sm text-gray-500">{p.payslip_no}</p>
                </div>
                <span className="text-sm font-semibold text-blue-600">{formatCurrency(p.net_salary)}</span>
              </div>
              ...
              <div className="mt-3 flex justify-end gap-2">
                ...
                {isAdmin() && (
                  <button
                    onClick={() => setDeleteModal({ show: true, id: p.id, name: p.employee_name })}
